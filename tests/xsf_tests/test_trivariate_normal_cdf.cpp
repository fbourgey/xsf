...
TEST_CASE("trivariate normal CDF test", "[trivariate_normal_cdf][xsf_tests]") {
    ...
    SECTION("trivariate normal CDF infinite inputs") {
        ...
        // CDF is evaluated as bivariate_normal_sf(-h1, -h2, r).
        ...
        auto [dh, dk, dl, r_xy, r_xz, r_yz, expected, rtol] = GENERATE(
            ...
            test_case{
                ...
                xsf::bivariate_normal_sf(-1.0, -2.0, 0.4), 1e-13
            },
            test_case{
                1.0, std::numeric_limits<double>::infinity(), 2.0, 0.2, -0.3, 0.4,
                xsf::bivariate_normal_sf(-1.0, -2.0, -0.3), 1e-13
            },
            test_case{
                1.0, 2.0, std::numeric_limits<double>::infinity(), 0.2, -0.3, 0.4,
                xsf::bivariate_normal_sf(-1.0, -2.0, 0.2), 1e-13
            }
        );
    ...
    SECTION("trivariate normal singular correlations") {
        ...
        const double expected =
            sign > 0.0
                ? xsf::bivariate_normal_sf(-h1, -std::min(h2, h3), rho)
                : (h2 > -h3 ? xsf::bivariate_normal_sf(-h1, -h2, rho) - xsf::bivariate_normal_sf(-h1, h3, rho) : 0.0);
        CAPTURE(rho, h3, sign);
