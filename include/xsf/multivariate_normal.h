...
trivariate_normal_cdf(double h1, double h2, double h3, double r12, double r13, double r23, double epsi = 1e-7) {
    ...
    if (h1 == math_inf) {
        ...
        return bivariate_normal_sf(-h2, -h3, r23);
    }
    if (h2 == math_inf) {
        if (h3 == math_inf) {
            return cephes::ndtr(h1);
        }
        return bivariate_normal_sf(-h1, -h3, r13);
    }
    if (h3 == math_inf) {
        return bivariate_normal_sf(-h1, -h2, r12);
    }
    ...
    if (std::abs(h1) + std::abs(h2) + std::abs(h3) < epst) {
        ...
        tvn = cephes::ndtr(h1) * bivariate_normal_sf(-h2, -h3, r23);
    } else if (std::abs(r13) + std::abs(r23) < epst) {
        tvn = cephes::ndtr(h3) * bivariate_normal_sf(-h1, -h2, r12);
    } else if (std::abs(r12) + std::abs(r23) < epst) {
        tvn = cephes::ndtr(h2) * bivariate_normal_sf(-h1, -h3, r13);
    } else if (1.0 - r23 < epst) {
        tvn = bivariate_normal_sf(-h1, -(h2 < h3 ? h2 : h3), r12);
        ...
        tvn = bivariate_normal_sf(-h1, -h2, r12) - bivariate_normal_sf(-h1, h3, r12);
    } else {
        ...
        tvn += bivariate_normal_sf(-h2, -h3, r23) * cephes::ndtr(h1);
    }
